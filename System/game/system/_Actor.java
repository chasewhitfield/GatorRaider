...
public abstract class _Actor implements Actor
{
    ...
    protected Actor getTargetActor(List<? extends Actor> targets, boolean nearest,
                                   boolean canReverse)
    {
        if (nearest) {
            ...
            for (int i = 0; i < targets.size(); i++) {
                int currentDistance =
                        targets.get(i).getLocation().getPathDistance(getLocation());
    ...
    protected Node getTargetNode(List<Node> targets, boolean nearest, boolean
            canReverse)
    {
        if (nearest) {
            int minDistance = Integer.MAX_VALUE;
            ...
            for (int i = 0; i < targets.size(); i++) {
                int currentDistance =
                        targets.get(i).getPathDistance(getLocation());
